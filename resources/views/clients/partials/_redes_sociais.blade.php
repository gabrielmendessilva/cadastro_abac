...
<div x-data="{ openRede: false, editOpen: null }" class="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <div class="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                    ...
                    @can('clients.edit')
                        <div class="flex gap-1">
                            <button type="button" @click="editOpen = {{ $rede->id }}"
                                    class="rounded-lg border border-amber-200 px-2 py-0.5 text-xs text-amber-700 hover:bg-amber-50">Editar</button>
    ...
    @can('clients.edit')
        ...
        {{-- Modais de edição --}}
        @foreach ($client->redesSociais as $rede)
            <div x-show="editOpen === {{ $rede->id }}" x-cloak class="fixed inset-0 z-[9999] overflow-y-auto bg-black/50">
                <div class="flex min-h-full items-center justify-center p-4">
                    <div @click.away="editOpen = null" class="w-full max-w-xl rounded-2xl bg-white shadow-2xl">
                        <form method="POST" action="{{ route('clients.redes.update', [$client, $rede]) }}">
                            @csrf @method('PUT')
                            <div class="border-b border-slate-200 px-6 py-4">
                                <h3 class="text-lg font-semibold text-slate-900">Editar rede social</h3>
                            </div>
                            <div class="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
                                <div>
                                    <label class="mb-1 block text-sm font-medium text-slate-700">Tipo *</label>
                                    <select name="tipo" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                                        @foreach (\App\Models\ClientRedeSocial::TIPOS as $key => $label)
                                            <option value="{{ $key }}" {{ $rede->tipo === $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="mb-1 block text-sm font-medium text-slate-700">Rótulo</label>
                                    <input type="text" name="rotulo" value="{{ $rede->rotulo }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="mb-1 block text-sm font-medium text-slate-700">URL *</label>
                                    <input type="url" name="url" value="{{ $rede->url }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                                </div>
                            </div>
                            <div class="flex justify-end gap-3 border-t border-slate-200 px-6 py-4">
