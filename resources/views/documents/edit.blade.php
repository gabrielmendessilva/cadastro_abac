...
@section('content')
<div class="rounded-3xl bg-white p-6 shadow-sm border border-slate-200"><form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data">@method('PUT')
@csrf
...
<div class="mt-6 flex gap-3"><button class="rounded-2xl bg-slate-900 px-5 py-3 text-white">Salvar</button><a href="{{ route('documents.index') }}" class="rounded-2xl border px-5 py-3">Cancelar</a></div>
</form></div>
@endsection